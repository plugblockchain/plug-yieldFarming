...
import { CommunityVault, ERC20Mock, Staking, PlugYieldFarm } from "../typechain";

describe('Plug YieldFarm Liquidity Pool', function () {
    let staking: Staking;
    let plugToken: ERC20Mock;
    let uniLP: ERC20Mock;
    let communityVault: CommunityVault;
    let yieldFarm: PlugYieldFarm;
    let creator: Signer, user: Signer;
    let userAddr: string;

    const epochStart = Math.floor(Date.now() / 1000) + 1000;
    const epochDuration = 1000;
    const numberOfEpochs = 25;
    ...
    before(async function () {
        ...
        plugToken = (await deployContract("ERC20Mock")) as ERC20Mock;
        uniLP = (await deployContract("ERC20Mock")) as ERC20Mock;

        communityVault = (await deployContract("CommunityVault", [plugToken.address])) as CommunityVault;
        yieldFarm = (await deployContract("PlugYieldFarm", [
            plugToken.address,
            uniLP.address,
            staking.address,
        ...
        await plugToken.mint(communityVault.address, distributedAmount);
    ...
    describe('General Contract checks', function () {
        it('should be deployed', async function () {
            ...
            expect(plugToken.address).to.not.equal(0)
        })
        ...
        it('Get epoch PoolSize and distribute tokens', async function () {
            ...
            expect(await plugToken.allowance(communityVault.address, yieldFarm.address)).to.equal(distributedAmount)
            expect(await yieldFarm.getCurrentEpoch()).to.equal(3)

            await yieldFarm.connect(user).harvest(1)
            expect(await plugToken.balanceOf(userAddr)).to.equal(distributedAmount.div(numberOfEpochs))
        })
    })
    ...
    describe('Contract Tests', function () {
        it('User harvest and mass Harvest', async function () {
            ...
            expect(await plugToken.balanceOf(userAddr)).to.equal(
            ...
            const totalDistributedAmount = amount.mul(distributedAmount.div(numberOfEpochs)).div(totalAmount).mul(8)
            expect(await plugToken.balanceOf(userAddr)).to.equal(totalDistributedAmount)
            expect(await yieldFarm.connect(user).userLastEpochIdHarvested()).to.equal(8)
            expect(await yieldFarm.lastInitializedEpoch()).to.equal(8) // epoch 8 have been initialized
        })
        it('Have nothing to harvest', async function () {
            ...
            expect(await plugToken.balanceOf(await creator.getAddress())).to.equal(0)
        ...
        it('it should return 0 if no deposit in an epoch', async function () {
            await moveAtEpoch(epochStart, epochDuration, 3)
            await yieldFarm.connect(user).harvest(1)
            expect(await plugToken.balanceOf(await user.getAddress())).to.equal(0)
        })
    })
