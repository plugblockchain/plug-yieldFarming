...
async function main () {
    ...
    const plugAmount = BigNumber.from(6_000_000).mul(tenPow18).mul(5).add(existingAllowanceForPlugPool); // monthly
    const plugEthlpAmount = BigNumber.from(7_000_000).mul(tenPow18).mul(5).add(existingAllowanceForPlugEthPool); // monthly
    const plugUsdclpAmount = BigNumber.from(7_000_000).mul(tenPow18).mul(5).add(existingAllowanceForPlugUsdcPool); // monthly
