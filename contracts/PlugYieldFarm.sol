...
contract PlugYieldFarm {
    ...
    // constructor
    constructor(address plugTokenAddress, address lpTokenAddress, address stakeContract, address communityVault) public {
    ...
    function massHarvest() external returns (uint){
        ...
        if (totalDistributedValue > 0) {
            _plug.transferFrom(_communityVault, msg.sender, totalDistributedValue);
        }
    ...
    function _getPoolSize(uint128 epochId) internal view returns (uint) {
        return _staking.getEpochPoolSize(_lpTokenAddress, _stakingEpochId(epochId));
    }

    // retrieve _lpTokenAddress token balance per user per epoch
    function _getUserBalancePerEpoch(address userAddress, uint128 epochId) internal view returns (uint){
        return _staking.getEpochUserBalance(userAddress, _lpTokenAddress, _stakingEpochId(epochId));
    }
